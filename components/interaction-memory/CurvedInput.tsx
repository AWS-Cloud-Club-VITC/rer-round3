...
const buildGeometry = (width: number, bend: number, thickness: number, pad: number): Geom => {
  const W = width;
  const T = thickness;
...
const fmt = (g: Geom, u: number, v: number) => {
  const [x, y] = g.point(u, v);
  return `${round2(x)} ${round2(y)}`;
};

const edgeSeg = (g: Geom, uTo: number, v: number, ltr: boolean) => {
...
const bentRectPath = (g: Geom, u0: number, u1: number, vTop: number, vBot: number, radius: number) => {
...
const bentLinePath = (g: Geom, u0: number, u1: number, v: number) =>
...
const CurvedInput = ({
  ...
  const svgStyle: React.CSSProperties | undefined = shadow
  ...
  if (geom && layout) {
    ...
    const buttonPath = showButton
      ? bentRectPath(geom, layout.btnU0, layout.btnU1, -T / 2 + layout.btnInset, T / 2 - layout.btnInset,
            ...
            {icon || (
              <>
                ...
                <path
                  ...
                  fill="none" stroke="#ffffff" strokeWidth={sw} strokeLinejoin="round" strokeLinecap="round"
                />
              </>
            )}
          </g>
